...
function verify_binder(gram, case, binder, non_binder=nothing)
    ...
    for ii in 1:(binder.size[1])
        ...
        etf_bool, a, b, c, dd = is_ETF(gram[simp_ind, simp_ind], case);
    ...
    if non_binder != nothing
        for ii in 1:(non_binder.size[1])
            ...
            etf_bool, a, b, c, dd = is_ETF(gram[nsimp_ind, nsimp_ind], case);
...
@testset "FiniteFieldFrames.jl" begin
    ...
    Phi = reconstruct_frame_from_gram(gram, "O");
    @test transpose(Phi)*Phi == Phi.base_ring.(gram);

    gram2 = real_etf_to_case_O(real_dx2d_etf(3^4), Int((3^4+1)/2), 7);
    Phi2 = reconstruct_frame_from_gram(gram2, "O");
...
@testset "BinderFinder.jl" begin
    ...
    gram = maximal_case_O_etf(7, 5);
    @test verify_binder(gram, "O", binder_finder(gram, "O"))

    gram2 = maximal_case_O_etf(7, 7);
    @test verify_binder(gram2, "O", binder_finder(gram2, "O"))

    gram3 = real_etf_to_case_O(real_dx2d_etf(5^2), Int((5^2+1)/2), 7);
    @test verify_binder(gram3, "O", binder_finder(gram3, "O"))
end
